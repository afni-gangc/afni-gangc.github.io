...
We should not forget our ultimate goal: estimating test-retest reliability! How to extract the information from the model output? All the information is embedded in the variance-covariance matrix \\(\mathbf \Sigma_{8\times 8}\\). So, here comes our finale (thanks to Michael Freund for providing an improvement upon my previous approach):
